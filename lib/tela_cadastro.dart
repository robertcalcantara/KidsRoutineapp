...
class _TelaCadastroState extends State<TelaCadastro> {
  ...
  bool emailInstitucionalValido(String email) =>
      email.toLowerCase().endsWith(dominioPermitido);
  ...
  Future<void> criarConta() async {
    ...
    if (!emailInstitucionalValido(emailController.text.trim())) {
      _snack('Use apenas email institucional @souunit.com.br');
      return;
    }
    ...
    try {
      ...
      switch (e.code) {
        case 'email-already-in-use': mensagem = 'Este email já está cadastrado'; break;
        case 'invalid-email':        mensagem = 'Email inválido'; break;
        case 'weak-password':        mensagem = 'Senha muito fraca'; break;
      }
      _snack(mensagem);
    } catch (e) {
      _snack('Erro: $e');
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Criar Conta', style: TextStyle(fontSize: 34, fontWeight: FontWeight.bold)),
              ...
              const SizedBox(height: 20),
              campoTexto(controller: confirmarSenhaController, texto: 'Confirmar senha', senha: true),
              const SizedBox(height: 20),
              campoTexto(controller: responsavel1Controller, texto: 'Nome do responsável'),
              const SizedBox(height: 20),
              campoTexto(controller: responsavel2Controller, texto: 'Segundo responsável (Opcional)'),
              const SizedBox(height: 20),
              campoTexto(controller: criancaController, texto: 'Nome da criança'),
              const SizedBox(height: 40),
